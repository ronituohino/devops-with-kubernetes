...
func AddTodo(text string, db *pg.DB) (*Todo, []Todo) {
	todo := &Todo{
		Todo: text,
		Done: false,
	}
	_, err := db.Model(todo).Returning("*").Insert()

	if err != nil {
		fmt.Println("Error adding todo", err)
	}

	return todo, GetTodos(db)
}

func MarkDone(id int64, db *pg.DB) (*Todo, []Todo) {
	todo := &Todo{
		Id:   id,
		Done: true,
	}

	_, err := db.Model(todo).Set("done = ?done").Where("id = ?id").Returning("*").Update()
